...
exports.getModelMapping = exports.ALIAS_MAP = exports.QODER_MODEL_BY_ID = exports.QODER_MODELS = void 0;
/**
...
exports.QODER_MODEL_BY_ID = Object.fromEntries(exports.QODER_MODELS.map((m) => [m.id, m]));
/**
...
const getModelMapping = (requestedModel) => {
    ...
    if (exports.QODER_MODEL_BY_ID[requestedModel])
        return requestedModel;
    // 2. Exact alias match
    if (exports.ALIAS_MAP[requestedModel])
        return exports.ALIAS_MAP[requestedModel];
    ...
    if (lower.includes('gemini')) {
        if (lower.includes('flash') || lower.includes('nano'))
            return 'efficient';
        return 'performance';
    }
    // Qwen family
    ...
    if (lower.includes('deepseek-v4-flash'))
        return 'dfmodel';
    if (lower.includes('glm-5.1'))
        return 'gm51model';
    if (lower.includes('qwen-3.6-plus'))
        return 'qmodel';
    // 4. Unknown model — warn and fall back to auto
    console.warn(`[model] Unknown model "${requestedModel}" — falling back to "auto". Add an alias in ALIAS_MAP to suppress this warning.`);
    return 'auto';
};
