...
describe('buildPromptWithTools', () => {
  ...
  test('generates prompt with tool instructions when tools provided', () => {
    ...
    const result = buildPromptWithTools(messages, tools as any, (msgs) => msgs.map(m => m.content).join('\n'));
  ...
  test('delegates to messagesToPrompt when tools is null', () => {
    const mockFn = jest.fn(() => 'delegated');
    const result = buildPromptWithTools([], null as any, mockFn);
    expect(result).toBe('delegated');
  });
  ...
  test('includes system message from messages', () => {
    ...
    const result = buildPromptWithTools(messages, tools as any, () => '');
    expect(result).toContain('System context: Be helpful');
  });

  test('handles multipart system content', () => {
    const messages = [
      { role: 'system' as const, content: [{ type: 'text', text: 'Sys msg' }] },
      { role: 'user' as const, content: 'Hi' },
    ];
    const result = buildPromptWithTools(messages, tools as any, () => '');
  ...
  test('includes tool role messages', () => {
    const messages = [
      { role: 'tool' as const, content: 'Tool result data' },
    ];
    const result = buildPromptWithTools(messages, tools as any, () => '');
...
describe('parseToolCallFromText', () => {
  ...
  test('returns null for null/undefined', () => {
    expect(parseToolCallFromText(null as any)).toBeNull();
    expect(parseToolCallFromText(undefined as any)).toBeNull();
  });
...
describe('toOpenAIToolCalls', () => {
  ...
  test('keeps string arguments as-is', () => {
    const toolCall = { name: 'fn', arguments: '{"a":1}' as any };
